...
int main() {
    ...
    while (chioce != 0) {
        ...
        printf("2. Delete Todo Item\n");
        printf("3. Alter Todo Item\n");
        printf("4. Display All Todo Items\n");
        printf("0. Exit\n");
...
bool FindItem(PtrL List){
    ...
    scanf("%d", &id);
    Ptr prve = SearchPrevItemById(List, id);
    if (List->Head->Id == id) {
...
bool DeleteItem(PtrL List){
    ...
    scanf("%d", &id);
    Ptr prev = SearchPrevItemById(List, id);
    Ptr temp = NULL;
    if(List->Head->Id == id) {
        temp = List->Head;
